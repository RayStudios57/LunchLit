...
export async function getUserRoleDisplay(userId: string, customRoles: { id: string; is_active?: boolean; priority?: number; display_name: string; icon?: string; color?: string }[]) {
  const { data: roles } = await supabase
    .from('user_roles')
    .select('role, custom_role_id')
    .eq('user_id', userId);
