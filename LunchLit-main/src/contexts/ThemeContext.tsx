...
import { isMatrixUnlocked } from '@/lib/secrets';

export type ThemeName = 'default' | 'ocean' | 'sunset' | 'forest' | 'lavender' | 'midnight' | 'summer' | 'matrix';
...
const themeColorSchemes: Record<ThemeName, { light: ThemeColors; dark: ThemeColors }> = {
  ...
  matrix: {
    light: {
      ...
      accent: '135 100% 55%',
      background: '140 30% 4%',
      foreground: '135 90% 70%',
      card: '140 30% 7%',
      cardForeground: '135 90% 75%',
      muted: '140 20% 12%',
      mutedForeground: '135 40% 55%',
      border: '135 60% 25%',
      themedBackground: '140 40% 3%',
    },
    dark: {
      primary: '135 90% 50%',
      accent: '135 100% 60%',
      background: '140 35% 3%',
      foreground: '135 95% 72%',
      card: '140 35% 6%',
      cardForeground: '135 95% 78%',
      muted: '140 25% 10%',
      mutedForeground: '135 45% 55%',
      border: '135 70% 28%',
      themedBackground: '140 45% 2%',
    },
  },
  summer: {
    light: {
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  const [useThemeBackground, setUseThemeBackgroundState] = useState(false);
  ...
  useEffect(() => {
    ...
    if (savedTheme && (themes.some(t => t.id === savedTheme) || (savedTheme === 'matrix' && isMatrixUnlocked()))) {
  ...
  useEffect(() => {
    if (user) {
      supabase
        ...
        .then(({ data }) => {
          if (data) {
            if (data.theme && data.theme !== 'summer') setThemeState(data.theme as ThemeName);
