...
import { Calculator, Plus, Trash2, Camera, Loader2 } from 'lucide-react';
...
export function GpaCalculatorWidget() {
  ...
  const handleScan = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 9 * 1024 * 1024) {
      toast({ title: 'Image too large', description: 'Please use an image under 9MB.', variant: 'destructive' });
      return;
    }
    setScanning(true);
    try {
      ...
      const { data, error } = await supabase.functions.invoke('parse-grades-photo', {
        body: { imageDataUrl: dataUrl },
      });
      if (error) throw error;
      const found: CourseEntry[] = (data?.courses ?? []).filter((c: any) => c.name && GRADE_POINTS[c.grade] !== undefined);
      if (found.length === 0) {
        toast({ title: 'No grades found', description: 'Try a clearer photo or different angle.' });
      } else {
        setCourses(found.map((c) => ({ name: c.name, grade: c.grade, credits: c.credits || 1 })));
        toast({ title: `Imported ${found.length} ${found.length === 1 ? 'class' : 'classes'}`, description: 'Review and adjust any mistakes.' });
      }
    } catch (err: any) {
  ...
  return (
    <Card className="card-elevated">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex items-center justify-between gap-2 p-3 rounded-lg border border-dashed border-primary/30 bg-primary/5">
          <div className="text-xs">
            <p className="font-medium text-foreground">📸 Scan grades from a photo</p>
            <p className="text-muted-foreground">Snap your report card and we'll fill it in.</p>
          </div>
          <input ref={fileRef} type="file" accept="image/*" capture="environment" onChange={handleScan} className="hidden" />
          <Button size="sm" variant="default" disabled={scanning} onClick={() => fileRef.current?.click()} className="shrink-0">
