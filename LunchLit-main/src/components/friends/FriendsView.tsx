...
import { Search, Users, Award, Crown, UserPlus, Heart, Check, X, PartyPopper, Calendar, CheckSquare, MessageCircle, ShieldOff, Eye, EyeOff, Star } from 'lucide-react';
import { ALL_BADGES_INCLUDING_MASTER, MASTER_BADGE } from '@/hooks/useAchievements';
import { useAuth } from '@/contexts/AuthContext';
import { useFriends } from '@/hooks/useFriends';
import { FriendChat } from './FriendChat';
import { useToast } from '@/hooks/use-toast';
import { usePresenceTracker, type PresenceStatus } from '@/hooks/usePresence';
...
export function FriendsView() {
  ...
  // Rich presence tracking for friends
  const friendIds = useMemo(() => 
    acceptedFriends.map(f => f.friend_profile?.user_id).filter(Boolean) as string[],
    [acceptedFriends]
  );
  const presenceMap = usePresenceTracker(friendIds);
  ...
  // Helper to get presence status dot color and label
  const getPresenceDot = (userId: string): { color: string; label: string; activity: string | null; lastSeen: number | null; customStatus: string | null } => {
    const p = presenceMap[userId];
    if (!p) return { color: 'bg-muted-foreground/40', label: 'Offline', activity: null, lastSeen: null, customStatus: null };
    if (p.status === 'idle') return { color: 'bg-yellow-500', label: 'Idle', activity: p.activity, lastSeen: p.last_seen, customStatus: p.custom_status };
    if (p.status === 'offline') return { color: 'bg-muted-foreground/40', label: 'Offline', activity: null, lastSeen: p.last_seen, customStatus: p.custom_status };
    return { color: 'bg-green-500', label: 'Online', activity: p.activity, lastSeen: null, customStatus: p.custom_status };
  };
  ...
  const { data: selectedShowcase = null } = useQuery({
    queryKey: ['user-showcase', selectedUserId],
    queryFn: async () => {
      if (!selectedUserId) return null;
      const { data, error } = await supabase
        .from('profiles')
        .select('showcase_badge')
        .eq('user_id', selectedUserId)
        .maybeSingle();
      if (error) return null;
      return data?.showcase_badge ?? null;
    },
    enabled: !!selectedUserId,
  });
  ...
  if (selectedUserId && selectedProfile) {
    ...
    const showcaseBadge = selectedShowcase
      ? ALL_BADGES_INCLUDING_MASTER.find(b => b.key === selectedShowcase && selectedBadges.includes(b.key))
      : null;
    return (
        ...
        <Card className="card-elevated">
          <CardContent className="py-8">
            <div className="flex flex-col items-center gap-4">
                ...
                <div className="flex items-center justify-center gap-2 flex-wrap">
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Badge variant={selectedProfile.is_public ? 'default' : 'outline'} className="gap-1 cursor-help">
                        {selectedProfile.is_public ? <Eye className="w-3 h-3" /> : <EyeOff className="w-3 h-3" />}
                        {selectedProfile.is_public ? 'Public' : 'Private'}
                      </Badge>
                    </TooltipTrigger>
                    <TooltipContent className="max-w-xs text-xs">
                      {selectedProfile.is_public
                        ? 'Visible in Student Profiles, badges & stats shown, name appears on community posts.'
                        : 'Hidden from Student Profiles & Friends discovery. Community posts appear anonymous. Badges & stats hidden from non-friends.'}
                    </TooltipContent>
                  </Tooltip>
                </div>
              ...
              {showcaseBadge && (
                <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-primary/10 ring-1 ring-primary/20">
                  <Star className="w-3.5 h-3.5 text-primary fill-primary" />
                  <span className="text-lg">{showcaseBadge.icon}</span>
                  <span className="text-sm font-medium">{showcaseBadge.name}</span>
                </div>
              )}
            ...
            <div className="grid gap-3">
              {acceptedFriends.map(f => (
                <Card key={f.id} className="card-interactive">
                  <CardContent className="p-4 flex items-center gap-3">
                    {(() => {
                      const fUserId = f.friend_profile?.user_id || '';
                      const presence = getPresenceDot(fUserId);
                      return (
                        <>
                          <div className="relative">
                            <Avatar className="cursor-pointer" onClick={() => setSelectedUserId(fUserId || null)}>
                              <AvatarImage src={f.friend_profile?.avatar_url || undefined} />
                              <AvatarFallback className="bg-primary text-primary-foreground">
                                {f.friend_profile?.full_name?.charAt(0) || '?'}
                              </AvatarFallback>
                            </Avatar>
                            <span className={`absolute bottom-0 right-0 w-3 h-3 rounded-full border-2 border-background ${presence.color}`} title={presence.label} />
                          </div>
                          <div className="flex-1 min-w-0 cursor-pointer" onClick={() => setSelectedUserId(fUserId || null)}>
                            <div className="flex items-center gap-2">
                              <p className="font-medium truncate">{f.friend_profile?.full_name || 'Anonymous'}</p>
                              <Badge 
                                variant="outline" 
                                className={`text-[10px] px-1.5 py-0 shrink-0 ${
                                  presence.label === 'Online' ? 'text-green-600 border-green-300' :
                                  presence.label === 'Idle' ? 'text-yellow-600 border-yellow-300' :
                            ...
                            <p className="text-xs text-muted-foreground truncate">
                              {presence.customStatus 
                                ? `"${presence.customStatus}"`
                                : presence.activity && presence.label !== 'Offline' 
                                  ? `In ${presence.activity}`
                                  : presence.label === 'Offline' && presence.lastSeen
                          ...
                          <div className="flex gap-1">
                            <Button variant="outline" size="sm" className="gap-1" onClick={() => setChatWith({ userId: fUserId, name: f.friend_profile?.full_name || 'Friend', avatar: f.friend_profile?.avatar_url })}>
                              <MessageCircle className="w-3.5 h-3.5" />
                            </Button>
                            {CHEER_OPTIONS.slice(0, 3).map(cheer => (
                              <Button key={cheer} variant="ghost" size="sm" className="text-lg p-1 h-8 w-8" onClick={() => sendCheer.mutate({ toUserId: fUserId, message: cheer })}>
                                {cheer.split(' ')[0]}
                              </Button>
                            ))}
