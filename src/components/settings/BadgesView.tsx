...
export function BadgesView() {
  ...
  const showcaseKey = profile?.showcase_badge ?? null;
  const showcaseBadge = ALL_BADGES_INCLUDING_MASTER.find(b => b.key === showcaseKey && unlocked.includes(b.key));

  const setShowcase = (key: string | null) => {
    updateProfile.mutate({ showcase_badge: key });
  };
    ...
    <div className="space-y-6 max-w-2xl mx-auto animate-fade-up">
      ...
      <Card className="card-elevated">
        <CardHeader className="pb-3">
          ...
          <CardDescription>
            Pick one unlocked badge to show off on your profile.
          </CardDescription>
        </CardHeader>
        <CardContent>
          {showcaseBadge ? (
            <div className="flex items-center gap-4">
              <div className="w-14 h-14 rounded-2xl flex items-center justify-center bg-primary/10 ring-1 ring-primary/30">
                <span className="text-3xl">{showcaseBadge.icon}</span>
              </div>
              <div className="flex-1">
                <p className="font-display font-semibold">{showcaseBadge.name}</p>
                <p className="text-xs text-muted-foreground">{showcaseBadge.description}</p>
              </div>
              <Button variant="ghost" size="sm" onClick={() => setShowcase(null)}>
                Remove
              </Button>
            </div>
          ) : (
            <p className="text-sm text-muted-foreground">
              No badge showcased yet — tap any unlocked badge below to feature it.
            </p>
          )}
        </CardContent>
      </Card>
              ...
              <CardContent>
                <div className="grid grid-cols-3 sm:grid-cols-4 gap-3">
                  {catBadges.map(badge => {
                    ...
                    const isShowcased = showcaseKey === badge.key;
                    const isSecretLocked = badge.secret && !isUnlocked;
                    return (
                      <Tooltip key={badge.key}>
                        <TooltipTrigger asChild>
                          <button
                            type="button"
                            disabled={!isUnlocked}
                            onClick={() => isUnlocked && setShowcase(isShowcased ? null : badge.key)}
                            className={`relative flex flex-col items-center gap-1.5 p-3 rounded-xl transition-all ${
                              isUnlocked
                                ? 'bg-primary/10 ring-1 ring-primary/20 hover:ring-primary/40 cursor-pointer'
                                : 'bg-secondary/30 opacity-50 grayscale hover:opacity-70 cursor-default'
                            } ${isShowcased ? 'ring-2 ring-primary' : ''}`}
                          >
                            {isShowcased && (
                              <Star className="absolute top-1.5 right-1.5 w-3.5 h-3.5 text-primary fill-primary" />
                            )}
                            <span className="text-3xl">{isSecretLocked ? '❓' : badge.icon}</span>
                        ...
                        <TooltipContent side="top" className="max-w-[220px]">
                          {isSecretLocked ? (
                            <>
                              <p className="font-medium">??? — Secret Badge</p>
                              <p className="text-xs text-muted-foreground">
                                Hidden until you discover how to unlock it.
                              </p>
                              <p className="text-xs text-muted-foreground mt-1">🔒 Criteria hidden</p>
                            </>
                          ) : (
                            <>
                              ...
                              {isUnlocked ? (
                                <p className="text-xs text-primary mt-1">
                                  {isShowcased ? '⭐ Showcased — tap to remove' : '✓ Unlocked — tap to showcase'}
                                </p>
                              ) : (
