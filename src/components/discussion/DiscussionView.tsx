...
// Bot accounts used to seed community question posts — always render anonymously
const BOT_USER_IDS = new Set([
  'b07b0001-0000-0000-0000-000000000001',
  'b07b0002-0000-0000-0000-000000000002',
  'b07b0003-0000-0000-0000-000000000003',
  'b07b0004-0000-0000-0000-000000000004',
]);
...
// Fetch public profile info for a set of author user_ids
function usePublicAuthors(userIds: string[]) {
  return useQuery({
    queryKey: ['discussion-public-authors', [...userIds].sort().join(',')],
    queryFn: async () => {
      const map: Record<string, { full_name: string | null; avatar_url: string | null; grade_level: string | null; school_name: string | null }> = {};
      if (userIds.length === 0) return map;
      const realIds = userIds.filter(id => !BOT_USER_IDS.has(id));
      if (realIds.length === 0) return map;
      const { data } = await supabase
        .from('profiles')
        .select('user_id, full_name, avatar_url, grade_level, school_name')
        .in('user_id', realIds)
        .eq('is_public', true);
      for (const p of data || []) {
        map[p.user_id] = {
...
export function DiscussionView() {
  ...
  const [peekedUserId, setPeekedUserId] = useState<string | null>(null);
  ...
  const { data: publicAuthors = {} } = usePublicAuthors(discussionUserIds);
  ...
  return (
    <div className="space-y-4 animate-fade-up">
      ...
      <div className="flex items-center gap-2 text-sm text-muted-foreground bg-secondary/50 p-3 rounded-lg">
        ...
        <span>Your posts appear anonymously unless your profile is set to Public in Settings.</span>
      </div>
            ...
            >
              <CardContent className="p-4">
                <div className="flex items-start gap-3">
                  {(() => {
                    const pub = publicAuthors[discussion.user_id];
                    const isAnon = !pub || BOT_USER_IDS.has(discussion.user_id);
                    const displayName = isAnon ? getAnonymousName(discussion.user_id) : (pub!.full_name || 'Student');
                    return (
                      <>
                        {isAnon ? (
                          <Avatar className="w-10 h-10" style={{ backgroundColor: getAnonymousAvatar(discussion.user_id) }}>
                            <AvatarFallback className="text-white">{displayName.charAt(0)}</AvatarFallback>
                          </Avatar>
                        ) : (
                          <Avatar className="w-10 h-10 cursor-pointer hover:ring-2 hover:ring-primary transition" onClick={(e) => { e.stopPropagation(); setPeekedUserId(discussion.user_id); }}>
                            <AvatarImage src={pub!.avatar_url || undefined} />
                            ...
                            <span className="flex items-center gap-1">
                              {isAnon ? (
                                ...
                                <button
                                  ...
                                  onClick={(e) => { e.stopPropagation(); setPeekedUserId(discussion.user_id); }}
                                >
                                  {displayName}
                                </button>
                              )}
      ...
      <ProfilePeekDialog userId={peekedUserId} onClose={() => setPeekedUserId(null)} />
    </div>
  );
}

function ProfilePeekDialog({ userId, onClose }: { userId: string | null; onClose: () => void }) {
  const { data: profile } = useQuery({
    queryKey: ['profile-peek', userId],
    queryFn: async () => {
      if (!userId) return null;
      const { data } = await supabase
        .from('profiles')
        .select('full_name, avatar_url, grade_level, school_name, is_public, custom_status')
        .eq('user_id', userId)
        .eq('is_public', true)
        .maybeSingle();
      return data;
          ...
          <div className="flex flex-col items-center gap-3 py-4">
            ...
            <div className="text-center">
              <div className="flex items-center justify-center gap-2 flex-wrap">
                <h3 className="font-display text-xl font-semibold">{profile.full_name || 'Student'}</h3>
                {userId === OWNER_USER_ID && (
                  <Badge className="bg-gradient-to-r from-yellow-400 to-amber-500 text-white gap-1 text-[10px]">Owner</Badge>
                )}
                <Badge variant="default" className="gap-1 text-[10px]">
                  <Eye className="w-3 h-3" /> Public
                </Badge>
              </div>
              {profile.grade_level && <p className="text-sm text-muted-foreground mt-1">{profile.grade_level}</p>}
              {profile.school_name && <p className="text-xs text-muted-foreground">{profile.school_name}</p>}
              {profile.custom_status && <p className="text-sm mt-2 italic">"{profile.custom_status}"</p>}
            </div>
          </div>
        ) : (
          <p className="text-sm text-muted-foreground text-center py-6">Profile unavailable.</p>
        )}
      </DialogContent>
    </Dialog>
  );
