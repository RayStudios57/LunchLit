  ...
  const { data: pendingEntries = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return (data || []).map((entry: Record<string, unknown>) => ({
        ...entry,
        student_name: (entry.profiles as { full_name?: string } | null)?.full_name || 'Unknown Student',
        student_grade: (entry.profiles as { grade_level?: string } | null)?.grade_level || 'Unknown Grade',
