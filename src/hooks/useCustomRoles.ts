  ...
  const { data: customRoles = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return (data || []).map(role => ({
        ...
        icon: ((role as Record<string, unknown>).icon as string) || 'shield',
        priority: ((role as Record<string, unknown>).priority as number) ?? 0,
        is_active: ((role as Record<string, unknown>).is_active as boolean) ?? true,
